...
export type Decorator = (cls: unknown, name: keyof any, descriptor: PropertyDescriptor) => PropertyDescriptor
export type WrappedFunction<
  F extends (...args: unknown[]) => unknown = (...args: unknown[]) => unknown
> = F & {inner: F}

// tslint:disable-next-line: no-any typedef
export default function decorator<W extends <F extends (...args: any[]) => any>(fn: F, ...rest: any[]) => F>(wrapper: W) {
  // tslint:disable-next-line: no-unnecessary-type-annotation
  return (...args: Rest1Type<W>): Decorator => (cls, name, descriptor): PropertyDescriptor => {
    return {
      ...descriptor,
...
>(
  wrapper: W, inner: F, ...wrapperArgs: Rest1Type<W>
): F & {inner: F} {
...
>(
  ...
  const wrapped = wrapperArgs.length > 0 ? wrapper(inner, ...wrapperArgs) : wrapper(inner)
...
export function proxied<T, N extends keyof T, W extends (...args: any[]) => unknown>(
  target: {prototype: T}, targetName: N, wrapped: W, name: string = keyToName(targetName)
): W & {
  inner: T[N];
} {
  try {
  ...
  Object.defineProperty(wrapped, 'inner', {
    configurable: true,
    get(): T[N] { return target.prototype[targetName] }
  })
